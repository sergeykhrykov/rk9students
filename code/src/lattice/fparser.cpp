...
FunctionParser::FunctionParser() :
	parseErrorType(NO_SYNTAX_ERROR), evalErrorType(0),
	data(new Data)
{
	data->referenceCounter = 1;
	using_var = (bool*)NULL;
...
FunctionParser::Data::~Data()
{
	delete[] ByteCode; ByteCode = 0;
	delete[] Immed; Immed = 0;
	...
	Stack(0), StackSize(cpy.StackSize)
{
	...
	if (ImmedSize) Immed = new float[ImmedSize];
	if (StackSize) Stack = new float[StackSize];
...
// Constants:
bool FunctionParser::AddConstant(const string& name, float value)
{
	if (isValidName(name))
	{
	...
	bool useDegrees)
{
	...
	parseErrorType = NO_SYNTAX_ERROR;
...
FunctionParser::FindVariable(const char* F, const Data::VarMap_t& vars) const
{
	if (vars.size())
	{
		...
		{
			string name(F, ind);
			FunctionParser::Data::VarMap_t::const_iterator vIter = vars.find(name);
			if (vIter != vars.end())
				using_var[vIter->second - VarBegin] = true;
			return vIter;
		}
	}
	return vars.end();
...
bool FunctionParser::Compile(const char* Function)
{
	...
	delete[] data->Stack; data->Stack = 0;

	vector<unsigned> byteCode; byteCode.reserve(1024);
	tempByteCode = &byteCode;

	vector<float> immed; immed.reserve(1024);
	tempImmed = &immed;
	...
	if (data->ImmedSize)
	{
		...
		memcpy(data->Immed, &immed[0],
			sizeof(float)*data->ImmedSize);
	}
	if (data->StackSize)
		data->Stack = new float[data->StackSize];
...
inline void FunctionParser::AddImmediate(float i)
{
...
int FunctionParser::CompileElement(const char* F, int ind)
{
	...
	if (isdigit(c) || c == '.' || c == '-') // Number
	{
		...
		char* endPtr;
		float val = (float)strtod(startPtr, &endPtr);
		AddImmediate(val);
...
float FunctionParser::Eval(const float* Vars)
{
	const unsigned* const ByteCode = data->ByteCode;
	const float* const Immed = data->Immed;
	float* const Stack = data->Stack;
	...
	for (IP = 0; IP<ByteCodeSize; ++IP)
	{
		...
		case   cCot:
		{
			float t = tan(Stack[SP]);
			if (t == 0) { evalErrorType = 1; return 0; }
			Stack[SP] = 1 / t; break;
		}
		case   cCsc:
		{
			float s = sin(Stack[SP]);
		...
		{
			data->Stack = new float[data->StackSize];
			float retVal = Eval(&Stack[SP - data->varAmount + 1]);
			delete[] data->Stack;
			data->Stack = Stack;
			SP -= data->varAmount - 1;
			Stack[SP] = retVal;
...
#define CONSTANT_E     (float)2.71828182845904509080  // exp(1)
#define CONSTANT_PI    (float)M_PI                    // atan2(0,-1)
#define CONSTANT_L10   (float)2.30258509299404590109  // log(10)
#define CONSTANT_L10I  (float)0.43429448190325176116  // 1/log(10)
#define CONSTANT_L10E  (float)CONSTANT_L10I           // log10(e)
#define CONSTANT_L10EI (float)CONSTANT_L10            // 1/log10(e)
#define CONSTANT_DR    (float)(180.0 / M_PI)          // 180/pi
#define CONSTANT_RD    (float)(M_PI / 180.0)          // pi/180
...
namespace {
	...
	public:
		...
		compres(char c) : state(c) {}
		// is it?
		operator bool() const { return state != 0; }
		// is it not?
		bool operator! () const { return state != 1; }
		bool operator==(bool b) const { return state != (char)!b; }
		bool operator!=(bool b) const { return state != (char)b; }
	private:
		char state;
	};
	...
	public:
		...
		void SetVar(unsigned val)
		{
			args.clear();
			op = cVar;
			var = val;
		}
		void SetImmed(float val)
		{
			args.clear();
			op = cImmed;
			value = orig = val;
...
#define CHECKCONSTNEG(item, op) \
	...
	private:
		...
		/*
		template<unsigned val> inline void chk() const
		{
		}
		*/
	...
	public:
		...
		// set
		void SetImmed(float val) { data->SetImmed(val); }
		void SetOp(unsigned op) { data->SetOp(op); }
		void SetVar(unsigned val) { data->SetVar(val); }
		// get
		float GetImmed() const { return data->GetImmed(); }
		...
		compres NonZero() const {
			if (!IsImmed()) return maybe;
			return GetImmed() != 0.0;
		}
		compres IsPositive() const {
			if (!IsImmed()) return maybe;
			return GetImmed() > 0.0;
		}
	...
	public:
		CodeTree() {}
		CodeTree(float val) { SetImmed(val); }
	...
	private:
		...
		void ReplaceWithConst(float value)
		{
			SetImmed(value);
		...
		void OptimizeRedundant()
		{
			...
			OptimizefloatNegations();
		}

		void OptimizefloatNegations()
		{
			if (GetOp() == cAdd)
			{
				// Eschew float negations
...
#define ConstantBinaryFun(token, fun) \
			...
			Redo:
				for (pit a = GetBegin(); a != GetEnd(); ++a)
				{
					...
					if (poslist.size() >= min)
					{
						...
						float factor = poslist.size() + 1.0f;
	...
	CodeTree::ConstList CodeTree::BuildConstList()
	{
		ConstList result;
		result.value =
			result.voidvalue = GetOp() == cMul ? 1.0f : 0.0f;
